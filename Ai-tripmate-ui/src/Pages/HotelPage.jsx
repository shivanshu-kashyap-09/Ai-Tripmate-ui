...
function HotelPage() {
    ...
    const fetchHotels = async () => {
      try {
        const response = await axios.post(
          "http://tomcat.localhost:8080/hotel/name",
      ...
      try {
        for (let i = 0; i < hotelNames.length; i++) {
          const response = await axios.get(
            `http://tomcat.localhost:8080/hotel/description/${hotelNames[i]}" "${userLocation}`
          );
          if (response.status === 200) {
  ...
  useEffect(() => {
    const fetchPrice = async () =>{
      try {
        for(let i = 0 ; i < hotelNames.length ; i++){
          const response = await axios.get(`http://tomcat.localhost:8080/hotel/price/${hotelNames[i]}" "${userLocation}" under "${userBudget}"`);
  ...
  useEffect(() => {
    const fetchRating = async () => {
      try {
        for(let i = 0 ; i < hotelNames.length ; i++){
          const response = await axios.get(`http://tomcat.localhost:8080/hotel/rating/${hotelNames[i]}" "${userLocation}`);
          if(response.status == 200 && response.data.body){
  ...
  useEffect(() => {
    const fetchImages = async () => {
      try {
        const imagesArray = [];
        for(let i = 0 ; i < hotelNames.length ; i++){
          const response = await axios.get(`http://tomcat.localhost:8080/ai/image-search?query=${hotelNames[i]}" "${userLocation}`);
  ...
  return (
    <div>
        ...
        images={hotelImages[0] || [hellojava, hellojava, hellojava]} 
        ...
        images={hotelImages[1] || [hellojava, hellojava, hellojava]}
        ...
        images={hotelImages[2] || [hellojava, hellojava, hellojava]}
        ...
        images={hotelImages[3] || [hellojava, hellojava, hellojava]}
        ...
        images={hotelImages[4] || [hellojava, hellojava, hellojava]}
        ...
        images={hotelImages[5] || [hellojava, hellojava, hellojava]}
        ...
        images={hotelImages[6] || [hellojava, hellojava, hellojava]}
        ...
        images={hotelImages[7] || [hellojava, hellojava, hellojava]}
        ...
        images={hotelImages[8] || [hellojava, hellojava, hellojava]}
        ...
        images={hotelImages[9] || [hellojava, hellojava, hellojava]}
