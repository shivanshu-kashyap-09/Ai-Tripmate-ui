...
public class HotelService {
    ...
    private JournalEntity validateAndUpdateHotelData(JournalEntity hotelData, String price) {
        ...
        if (hotelData.getPriceRange() == null || hotelData.getPriceRange().trim().isEmpty()) {
            ...
            priceFuture = CompletableFuture.supplyAsync(() -> fetchService.getData(
                    "Provide only the average price range of " + hotelData.getExploreName() + " hotel under " + price + ". No extra text."));
        }
        ...
        if (hotelData.getRating() == null || hotelData.getRating().trim().isEmpty()) {
            ...
            ratingFuture = CompletableFuture.supplyAsync(() -> fetchService.getData(
                    "Provide only the Google review rating of " + hotelData.getExploreName() + " hotel. No extra text."));
        }

        if (hotelData.getImage() == null || hotelData.getImage().trim().isEmpty()) {
            needsUpdate = true;
            imageFuture = CompletableFuture.supplyAsync(() -> imageController.searchImages(hotelData.getExploreName()));
        }
        ...
        if (needsUpdate) {
            ...
            if (imageFuture != null) hotelData.setImage(imageFuture.join());
    ...
    private JournalEntity fetchAndSaveHotelData(String hotelName, String price) {
        ...
        CompletableFuture<String> imageFuture = CompletableFuture.supplyAsync(() -> imageController.searchImages(hotelName));
